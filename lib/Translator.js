...
class Translator {
    ...
    async start() {
        try {
            ...
            kafkaClient.runLoops();
            // setTimeout(() => kafkaClient.producer.sendMessages(counter, topicName, 0) , 10000);



            setTimeout( () => kafkaClient.producer.produce(kafkaClient.producer.topicName, null, nva.id, '{ "foo": "bar" }'), 10000);
    ...
    async _processCoreNVAMessage({ nva }) {
        ...
        kafkaClient.producer.produce(kafkaClient.producer.topicName, null, nva.id, JSON.stringify(nva));
